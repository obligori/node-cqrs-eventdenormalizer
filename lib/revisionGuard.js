...
const debug = require('debug')('denormalizer:revisionGuard');

const _ = require('lodash');
const dotty = require('dotty');

const Queue = require('denormalizer/lib/orderQueue');
const ConcurrencyError = require('denormalizer/lib/errors/concurrencyError');
const AlreadyDenormalizedError = require('denormalizer/lib/errors/alreadyDenormalizedError');
const AlreadyDenormalizingError = require('denormalizer/lib/errors/alreadyDenormalizingError');
...
class RevisionGuard {
  constructor (store, options) {
    ...
    if (!store || !_.isObject(store)) {
      const err = new Error('store not injected!');
      debug(err);
      throw err;
    }
   ...
   */
  queueEvent (evt, callback) {
    const self = this;
    const evtId = dotty.get(evt, this.definition.id);
    const revInEvt = dotty.get(evt, this.definition.revision);
    const aggId = dotty.get(evt, this.definition.aggregateId);

    const concatenatedId = this.getConcatenatedId(evt);
   ...
   *                              `function(err){}`
   */
  finishGuard (evt, revInStore, callback) {
    const evtId = dotty.get(evt, this.definition.id);
    const revInEvt = dotty.get(evt, this.definition.revision);
    ...
    this.store.set(concatenatedId, revInEvt + 1, revInStore, function (err) {
      if (err) {
        ...
        if (err instanceof ConcurrencyError) {
          const retryIn = randomBetween(0, self.options.retryOnConcurrencyTimeout || 800);
      ...
      const pendingEvents = self.queue.get(concatenatedId);
      ...
      const nextEvent = _.find(pendingEvents, function (e) {
        const revInNextEvt = dotty.get(e.payload, self.definition.revision);
  ...
  guard (evt, callback) {
    if (!this.definition.aggregateId || !dotty.exists(evt, this.definition.aggregateId) ||
      ...
      const err = new Error('Please define an aggregateId!');
      debug(err);
      return callback(err);
    }
    ...
    process.nextTick(function () {
      self.store.get(concatenatedId, function (err, revInStore) {
        ...
        if (!revInStore && revInEvt !== (self.options.startRevisionNumber || 1) && !self.currentHandlingRevisions[concatenatedId]) {
          let max = (self.options.queueTimeout * self.options.queueTimeoutMaxLoops) / 3;
          max = max < 10 ? 10 : max;
